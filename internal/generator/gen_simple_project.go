...
func (g *SimpleProjectGenerator) Generate(ctx context.Context, request CreateProjectRequest) (*bytes.Buffer, error) {
	...
	// internal/service_test.go — placeholder test
	if err := addToZip(zipWriter, fmt.Sprintf("%s/internal/service_test.go", folderName), GeneratePlaceholderTest("internal")); err != nil {
		log.Printf("[ERROR] %v", err)
		return nil, err
	}
