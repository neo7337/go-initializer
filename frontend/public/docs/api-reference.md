...
```json
{
  ...
  "supportedFrameworks": {
    ...
    "ai-agent":       { "langchaingo": true, "openai": true, "gemini": true, "ollama": true }
  },
  "supportedAddons": {
    ...
    "ai":          { "langchaingo": true },
    "vectorstore": { "pgvector": true, "chromem": true, "qdrant": true }
  }
}
```

> **`ai-agent` note:** For this project type, the `framework` field selects the LLM provider (`langchaingo`, `openai`, `gemini`, `ollama`) rather than an HTTP framework.
...
```json
{
  ...
  "moduleName":    "github.com/acme/my-agent",
  "name":          "my-agent",
  "description":   "LangChain-powered AI agent",
  "selectedAddons": {
...
| --- | --- | --- | --- |
| `projectType` | string | yes | `microservice` · `simple-project` · `cli-app` · `api-server` · `ai-agent` |
...
## Example: AI Agent with vector store
...
curl -X POST http://localhost:8182/api/generate \
  ...
  -d '{
    ...
    "moduleName": "github.com/acme/my-agent",
    "name": "my-agent",
    "selectedAddons": { "vectorstore": ["pgvector"] },
    "dockerSupport": true
  }' \
  --output my-agent.zip
```
